...
class HOIModel(nn.Module):
    ...
    #     :param patch_size: 特征图上的 patch 大小
    #     :return: ROIs 张量，形状为 [N, 5]，其中 N 是所有图片中检测框的总数
    #     """
    #     all_rois = []
    #     additional_info = []  # 用于存储每个 RoI 的标签、图像索引和分数
    ...
    #     rois_tensor = torch.cat(all_rois, dim=0).float()
    #     return rois_tensor, additional_info
...
class HOIModel_new(nn.Module):
    def __init__(self, backbone, device, num_objects=10, feature_dim=768, person_category_id=1, patch_size=14):
        super(HOIModel_new, self).__init__()
        self.backbone = backbone
        self.device = device
        self.num_objects = num_objects
        self.feature_dim = feature_dim
    ...
    def forward(self, nested_tensor: NestedTensor , targets, detections_batch):
        ...
        for b in range(batch_size):
            ...
            if len(detections) == 0: 
                ...
                continue
            detection_loop_start_time1 = time.time()
            ...
            bboxes_scaled = (bboxes / self.patch_size * 2).int()

            valid_bboxes_mask = (bboxes_scaled[:, 2] > bboxes_scaled[:, 0]) & (bboxes_scaled[:, 3] > bboxes_scaled[:, 1])
            valid_bboxes_mask &= (bboxes_scaled[:, 0] >= 0) & (bboxes_scaled[:, 1] >= 0) & (bboxes_scaled[:, 2] <= w) & (bboxes_scaled[:, 3] <= h)

            valid_bboxes = bboxes_scaled[valid_bboxes_mask]
            valid_detections = [detections[i] for i in range(len(detections)) if valid_bboxes_mask[i].item()]
            detection_loop_time1 = time.time() - detection_loop_start_time1
            print("Detection loop time1: ", detection_loop_time1)
            
            detection_loop_start_time = time.time()
            for det, bbox in zip(valid_detections, valid_bboxes):
                x1, y1, x2, y2 = bbox
