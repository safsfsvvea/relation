...
def draw_bbox(ax, bbox, label, score, color):
    # bbox 是 xyxy 形式的
    ...
    ax.add_patch(rect)
    # 绘制标签，同时包含类别和置信分数
    ax.text(x_min, y_min, f'{label}: {score:.2f}', verticalalignment='top', color=color, fontsize=8, weight='bold', bbox=dict(facecolor='white', alpha=0.5))
...
def visualize_annotations(filename, annotations, output_dir):
    ...
    # 绘制所有注释中的边界框、类别标签和置信分数
    for annotation in annotations:
        bbox = annotation['bbox']
        category_id = annotation['category_id']
        score = annotation['score']
        draw_bbox(ax, bbox, f'Cat: {category_id}', score, 'green')
