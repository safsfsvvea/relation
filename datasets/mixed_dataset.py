...
class MixedRelDetectionWithDet(torch.utils.data.Dataset):
    def __init__(self, img_set,
                       anno_file, 
                       detection_results,
                       transforms, 
    ...
    def __getitem__(self, idx):
        ...
        detections = self.detection_results.get(img_id, [])
        ...
        if self.img_set == 'pretrain':
            ...
            if self._transforms is not None:
                # print("before transform")
                # print("det['boxes']: ", detection['boxes'])
                # print("target['boxes']: ", target['boxes'])
                img, target, detection = self._transforms(img, target, detection) # target['boxes'].shape and target['labels'].shape may change
            ...
            detection['image_id'] = img_anno['image_id']
...
def build(image_set, args):
    ...
    if args.dataset_file == 'vg_coco2017_o365':
    ...
    elif args.dataset_file == 'coco2017_det':
        # assert args.use_all_text_labels
        assert image_set == 'pretrain'
        dataset_list = ['coco2017']
        coco2017_img_folder = Path(args.coco_path) # / 'train2017'
        ...
        # print("args.keep_names_freq_file: ", args.keep_names_freq_file)
        # print("args.use_all_text_labels: ", args.use_all_text_labels)
        # print("args.pair_overlap: ", args.pair_overlap)
        dataset = MixedRelDetectionWithDet(
                            img_set = image_set,
                            anno_file = args.coco_rel_anno_file,
                            detection_results= args.coco_det_file,
                            keep_names_freq_file = args.keep_names_freq_file,
