...
def crop_to_multiple_of(sample, multiple=14):
    """
    裁剪图像，使其宽度和高度成为指定倍数。
    """
    _, _, height, width = sample.tensors.shape
    new_height = (height // multiple) * multiple  # 计算新的高度为14的倍数
    new_width = (width // multiple) * multiple  # 计算新的宽度为14的倍数
    # 裁剪图像
    sample.tensors = sample.tensors[:, :, :new_height, :new_width]
    sample.mask = sample.mask[:, :new_height, :new_width]
    return sample, (new_height, new_width)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if hasattr(model.module.transformer, 'text_encoder'):
            ...
            # print("mask: ", samples.mask.shape)
            if(args.backbone == 'DINOv2'):
                samples, (new_height, new_width) = crop_to_multiple_of(samples)
                kwargs['new_height'] = new_height
                kwargs['new_width'] = new_width
            kwargs['backbone_name'] = args.backbone
            # print("samples after crop: ", samples.tensors.shape)
